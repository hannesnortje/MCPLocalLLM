    ...
    def handle_system_health(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.memory_manager:
                try:
                    if (hasattr(self.memory_manager, 'client') 
                    ...
                    if (hasattr(self.memory_manager, 'embedding_model') 
            ...
            for component, info in health_info["components"].items():
                if (isinstance(info, dict) 
                    and info.get("status") in ["unavailable", "error"]):
            ...
            if not component_issues:
                ...
                status_text = (
                    f"⚠️ Degraded: Issues with: {', '.join(component_issues)}"
                )
