...
from .memory.collection_manager import CollectionManager
...
class GenericMemoryService:
    ...
    async def create_collection(
        ...
        # NOTE: CollectionPermissions not available in migrated version
        # perm_obj = None
        # if permissions:
        ...
        return self.collection_manager.create_collection(
            ...
            category=category,
            project=project,
            # permissions=perm_obj,  # Not available in migrated version
            # created_by=self.current_user  # Not available in migrated version
        )
    ...
    async def delete_collection(self, name: str, confirm: bool = False) -> dict[str, Any]:
        """Delete a collection."""
        if not self._ensure_initialized():
            return {"success": False, "error": "Service not initialized"}
    ...
    async def add_memory(
        self, collection: str, content: str, metadata: dict[str, Any] = None, tags: list[str] = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if collections is None:
                ...
                if all_collections.get("success"):
                    collections = [col["name"] for col in all_collections["collections"]]
                else:
                    collections = []
            ...
            for collection_name in collections:
                try:
                    ...
                    collection_info = self.collection_manager.get_collection(collection_name)
                    if not collection_info.get("success"):
                        continue
    ...
    def query_memory(
        ...
        try:
            ...
            if not collection_names:
                return {
                    "success": False,
                    "error": (f"No valid collections found for memory types: " f"{memory_types}"),
                }

            # Search across collections using sync wrapper
            result = self._search_memory_sync(
                collection_names=collection_names, query=query, limit=limit, min_score=min_score
            )
                    ...
                    patterns.append(
                        {
                            ...
                            "pattern_type": memory["metadata"].get("pattern_type", "insight"),
    ...
    def _search_memory_sync(
        self, collection_names: list[str], query: str, limit: int = 10, min_score: float = 0.3
    ) -> dict[str, Any]:
