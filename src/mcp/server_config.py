...
class ConfigManager:
    ...
    def _validate_config(self):
        ...
        if not (0.0 <= self.config.deduplication.similarity_threshold <= 1.0):
            errors.append(
                f"Invalid similarity threshold: {self.config.deduplication.similarity_threshold}"
            )

        if not (0.0 <= self.config.memory.type_confidence_threshold <= 1.0):
            errors.append(
                f"Invalid confidence threshold: {self.config.memory.type_confidence_threshold}"
            )
