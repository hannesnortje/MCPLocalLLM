    ...
    def handle_system_health(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            health_info = {
                ...
                "memory_manager": {
                    ...
                    "initialized": (
                        ...
                        and getattr(self.memory_manager, "collections_initialized", False)
                    ),
                },
                "error_statistics": error_handler.get_error_stats(),
                "components": {"markdown_processor": self.markdown_processor is not None},
            }
            ...
            if self.memory_manager:
                try:
                    ...
                    if (
                        ...
                        health_info["components"]["embedding_model"] = {"status": "unavailable"}

                except Exception as e:
                    health_info["components"]["qdrant"] = {"status": "error", "error": str(e)}
            ...
            if not component_issues:
                ...
                status_text = (
                    f"❌ Critical: All components have issues: " f"{', '.join(component_issues)}"
                )
            else:
...
"""
            ...
            for component, info in health_info["components"].items():
                if isinstance(info, dict):
                    ...
                    health_text += (
                        f"- **{component.replace('_', ' ').title()}:** " f"{status_icon} {status}\n"
                    )
...
"""
    ...
    async def handle_create_collection(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not collection_name:
                return {
                    "isError": True,
                    "content": [{"type": "text", "text": "Collection name is required"}],
                }
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Successfully created collection '{collection_name}'"}
                ]
            }
        except Exception as e:
    ...
    async def handle_query_collection(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not collection_name or not query:
                return {
                    "isError": True,
                    "content": [{"type": "text", "text": "Collection name and query are required"}],
                }
            ...
            if not results:
                result_text = (
                    f"No results found in collection '{collection_name}' " f"for query: '{query}'"
                )
            else:
                result_text = (
                    f"Found {len(results)} results in collection " f"'{collection_name}':\n\n"
                )

                for i, result in enumerate(results, 1):
                    result_text += f"**Result {i}** " f"(Score: {result.get('score', 'N/A'):.3f})\n"
    ...
    async def handle_get_collection_stats(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "isError": True,
                "content": [{"type": "text", "text": f"Failed to get collection stats: {str(e)}"}],
            }
